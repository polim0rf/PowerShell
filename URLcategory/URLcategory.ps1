...
filter Categorize {
    ...
    If ($po.unrated -eq 'True') {
        ...
        $Payload = @{comments = $comment ; email1 = $email1; email2 = $email2; cat1 = $cat; cat2 = $cat2; referrer = $referrer; sendEmail = $sendEmail; trackid = ""}
        $json = $Payload | ConvertTo-Json
    ...
    Else {
        ...
        # If category not malicious, re-submit
        $po.categorization.num | foreach {
            If (($_ -eq '43') -or ($_ -eq '102') -or ($_ -eq '44') -or ($_ -eq '92') -or ($_ -eq '18')) {
                $categorized = "true"
            }
            Else {
                $categorized = "false"
        ...
        If ($categorized -eq "false") {

            Write-Host -Fore Cyan 'Sending re-categorization...'
